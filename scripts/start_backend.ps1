...
if (-not (Test-Path $venvPython)) {
    ...
    # En Windows, un venv copiado desde otra PC puede devolver error sin lanzar excepcion de PowerShell.
...
Invoke-CheckedCommand -Executable $venvPython -Args @("-m", "pip", "install", "--upgrade", "pip") -ErrorMessage "Fallo al actualizar pip"
...
Invoke-CheckedCommand -Executable $venvPython -Args @("-m", "uvicorn", "app.main:app", "--reload", "--host", "0.0.0.0", "--port", "8000") -ErrorMessage "Fallo al iniciar backend"